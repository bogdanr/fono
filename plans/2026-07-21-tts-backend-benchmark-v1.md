...
- Capture **quality inputs**: write normalized, blindly-named WAV files plus an
  index so the maintainer can listen and rank; add an **STT round-trip WER/CER**
  objective proxy to flag gross mispronunciation/dropped words.
...
- [ ] Task 1. **Add a `tts` subcommand to `fono-bench`.** Extend the `Cmd` enum
  ...
  `--machine-label`, `--blind` (anonymized filenames). Rationale: reuses the
  established bench CLI/report/wav plumbing and feature gating.
...
- [ ] Task 7. **WAV output, normalization, and blind naming.** Write each
  utterance to a WAV via `fono_bench::wav` / `fono_core::wav`. Apply
  **peak/RMS loudness normalization** in-house (no new crate) so listening isn't
  biased by level. In `--blind` mode, name files with an opaque id and emit a
  separate key file mapping id → (backend, voice, sentence) so A/B listening is
  unbiased. Provide a grouped listening **index** (Markdown or minimal HTML) that
  lets the maintainer play clips per sentence across backends. Rationale: the
  human quality assessment is the primary quality signal; make it easy and unbiased.
...
- [ ] Task 12. **Gates.** Run `cargo fmt --all -- --check`,
  ...
  `cargo test --workspace --tests --lib`, and (since binary shipped artefact is
...
- WAV files are written, loudness-normalized, and (in `--blind`) anonymized with a
  key file; a listening index groups clips by sentence across backends.
...
4. **Loudness normalization could need a new crate (e.g. `ebur128`).**
   Mitigation: use in-house peak/RMS normalization (no dependency); only flag
   `ebur128` for sign-off if true LUFS is later deemed necessary.
5. **Assets not present in the voices cache cause confusing failures.**
   Mitigation: explicit pre-flight ensure/verify step with actionable errors
   (Task 4) pointing at daemon startup download.
6. **Cold-start numbers polluted by OS disk cache warmth.**
...
7. **Peak-RSS via `/proc` is Linux-only.**
